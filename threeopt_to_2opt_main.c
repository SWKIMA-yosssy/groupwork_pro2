...
int two_opt(struct city *cities, int *new_path, int n, double *new_path_length,
            int whether_geograph) {
  int i, j;
  int improved = 1;
  int new_tour[n + 1];
...
int main(void) {
  ...
  while (1) {
    ...
    */
    ...
    // 2-optによる改善
    buf = two_opt(cities, new_path, N, &new_path_length, whether_geograph);
    if (buf != 0) {
      printf("2-opt after 3-opt work\n");
    }
    for (i = 0; i < N - 1; i++) { // ###FOR DEBUG
      if (new_path[i] == new_path[i + 1]) {
        if (overlap_count == 0) {
